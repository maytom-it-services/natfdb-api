    ...
    for (const op in endpoints) {
        ...
        for (const code in operationSpec.responses) {
            for (const c in operationSpec.responses[code].content) {
                ...
                if (tsType) {
                    content.schema = generate_schema_1.resolveSchema(tsType, content.schema);
                    // We don't want a Function type in the final spec.
                    delete content[TS_TYPE_KEY];
                }
                if (content.schema.type === 'array') {
                    content.schema.items = generate_schema_1.resolveSchema(content.schema.items[TS_TYPE_KEY], content.schema.items);
                    // We don't want a Function type in the final spec.
